...
AuthController.signUp = function(req, res) {
    const schema = Joi.object().keys({
        ...
        accType: Joi.string().valid(['Member', 'Pathak']).required(),
    ...
    return Joi.validate(req.body, schema, function (err, value) {
        if (err) {
            ...
            let paymentRequest = {};
            if (value.accType === 'Pathak') {
                const payment = new Insta.PaymentData();
                payment.purpose = `Aroha Pathak account fees: ${value.email}`;            // REQUIRED
        ...
        if (err) {
            ...
            return db.Users.findOne(potentialUser).then(function(user) {
                if(!user) {
                    ...
                    if (user.Pathak && !user.Pathak.isPaymentVerified) {
                        const payment = new Insta.PaymentData();
                        payment.purpose = `Aroha Pathak account fees: ${user.email}`;            // REQUIRED
                        payment.amount = 1000;                  // REQUIRED
                        ...
                        Insta.createPayment(payment, function(error, response) {
                            if (error) {
                                ...
                                console.log(paymentRequest);
                                user.Pathak.paymentRequestId = paymentRequest.payment_request.id;
                                user.Pathak.save().then(() => {
                                    return res.status(200).json({
                                        needPayment: true,
                                        url: paymentRequest.payment_request.longurl
                                    });
